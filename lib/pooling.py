...
class PoolingClassifier(BaseEstimator, ClassifierMixin):
    """
    ...
    def fit(self, X, y):
        ...
        # vocabulary size: all known words
        self.num_words = len(np.unique(X))

        # if early stop, then train_test_split
        if self.early_stop:
            self.X_, self.X_dev, self.y_, self.y_dev = train_test_split(X, y, test_size=0.1,
    ...
    def _init_params(self):
        ...
        self.word_lookup = self.model.add_lookup_parameters((self.num_words, 1, 1, self.word_embedding_size))

        self.softmax_weight = self.model.add_parameters((self.num_labels, self.word_embedding_size))
...
def parse_args():
    ...
    model_args.add_argument('--pooling', default="average", type=str,
                            dest="pooling_method", choices=["average", "max"],
